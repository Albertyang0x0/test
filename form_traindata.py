...
def get_label_map(datapack):
    filepath = "../" + datapack + "/training_label_" + datapack + ".csv"
    df = pd.read_csv(filepath)
    map = dict()
    for _, row in df.iterrows():
        map[row["id"]] = row["source"]
    return map

services = get_servece_map()
keywords = get_keyword_map()
labels = get_label_map(sys.argv[2])
...
def get_log_feature(datapack, id):
    try:
        ...
        data_frame["diff"] = data_frame["timestamp"].diff()
...
def get_trace_feature(datapack, id):
    try:
        ...
        diff_std_array = [0] * len(services)
        for _, row in diff_group_mean.iterrows():
            diff_mean_array[services[row["service_name"]]] = row["diff"]
        
        for _, row in diff_group_std.iterrows():
            diff_std_array[services[row["service_name"]]] = row["diff"]
...
total = len(idlist)
finish_count = 0
output = open("train_data_" + task_no + "_" + datapack + ".csv", "w")
for id in idlist:
    diff_mean_array, diff_std_array, count_array, uni_no, msg_len_mean, msg_len_std = get_log_feature(datapack, id)
    print(f"{labels[id]}\t{msg_len_std}\t{msg_len_mean}\t{uni_no}\t", end = '', file = output)
